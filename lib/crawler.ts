// lib/crawler.ts — compatible ES5/ES2015
...
function extractSchemas(html: string): { has_schema: boolean; schema_types: string[] } {
  ...
  const microdataRegex = /itemtype=["']https?:\/\/schema\.org\/([^"'\s]+)["']/gi
  let mdMatch: RegExpExecArray | null
  while ((mdMatch = microdataRegex.exec(html)) !== null) {
    if (!types.includes(mdMatch[1])) types.push(mdMatch[1])
  }
  return { has_schema: types.length > 0, schema_types: Array.from(new Set(types)) }
}
...
function extractHeadings(html: string): {
  ...
  while ((m = h2Regex.exec(html)) !== null) h2s.push(m[1].replace(/<[^>]+>/g, '').trim())
  const questionWords = ['comment','pourquoi','quand','quel','quelle','quels','quelles','est-ce','où','qui','how','why','what','when','where','which','who']
  const h2Questions = h2s.filter(h2 => h2.endsWith('?') || questionWords.some(w => h2.toLowerCase().startsWith(w)))
  const issues: string[] = []
  if (h1s.length === 0) issues.push('Aucun H1 sur la page')
  if (h1s.length > 1) issues.push(`${h1s.length} H1 détectés — 1 seul attendu`)
...
function hasFaq(html: string): boolean {
  return /faq|questions?\s+fr[eé]quentes?|foire\s+aux\s+questions?|frequently\s+asked|FAQPage/i.test(html)
}

function hasKeyFacts(html: string): boolean {
  const matches = html.match(/\d+\s*(m²|m2|km|€|\$|personnes?|chambres?|suites?)/gi)
...
export async function crawlSite(url: string): Promise<CrawlResult> {
  ...
  const [robots_txt_ok, sitemap_ok] = await Promise.all([checkRobotsTxt(url), checkSitemap(url)])
  const linkCount = (html.match(/<a[^>]+href=["'][^"']+["']/gi) || []).length
  return {
    url,
    has_schema: schema.has_schema, schema_types: schema.schema_types,
    h1_count: headings.h1_count, h2_count: headings.h2_count,
    h2_as_questions: headings.h2_as_questions, h2_as_questions_pct: headings.h2_as_questions_pct,
    heading_issues: headings.issues,
    images_total: images.images_total, images_missing_alt: images.images_missing_alt,
    images_duplicate_alt: images.images_duplicate_alt, alt_quality: images.alt_quality,
    title: meta.title, title_length: meta.title_length,
    meta_description: meta.meta_description, meta_description_length: meta.meta_description_length,
    robots_txt_ok, sitemap_ok, robots_tag: extractRobotsTag(html),
    has_faq: hasFaq(html), has_key_facts: hasKeyFacts(html),
    word_count: countWords(html), lang_tag: extractLang(html),
    crawlable_links: linkCount > 3,
  }
}
